...
def test_model(model: torch.nn.Module, dataloader: torch.utils.data.DataLoader, loss_fn = None, device = None, train_classes = None):
    '''
    Evaluates a model on a given dataloader. Will also run a per-class accuracy check.
    ...
    train_classes: A list of class labels from the training dataset (needed to map the test set classes to the correct labels).
    '''
    ...
    # Ensure correct mapping from class index to class label from the training dataset
    for (cl, corr), (_, num_items) in zip(num_correct_classes.items(), num_items_classes.items()):
        perf_per_class = corr / num_items
        # Use the correct class name from the train_classes list for the test dataset's classes
        punch_id = train_classes[cl] if train_classes is not None else dataloader.dataset.classes[cl]
